...
`,Fo={__name:`AhorraPage`,setup(e){let t=Bt(null),n,r,i=e=>{e.classList.add(`is-revealed`)},a=e=>{e.forEach(i)},o=()=>{let e=t.value;if(!e)return;let r=Array.from(e.querySelectorAll(`.ahorra-scope-como-funciona .step[data-scroll-step]`));if(!r.length)return;if(window.matchMedia(`(prefers-reduced-motion: reduce)`).matches||!(`IntersectionObserver`in window)){a(r);return}let o=0,s=()=>{for(;o<r.length;){let e=r[o];if(!e.classList.contains(`is-pending-reveal`))break;e.classList.remove(`is-pending-reveal`),i(e),o+=1}};n=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&(e.target.classList.add(`is-pending-reveal`),n?.unobserve(e.target))}),s()},{threshold:.65,rootMargin:`0px 0px -42% 0px`}),r.forEach(e=>{n?.observe(e)})},s=[`application/pdf`,`image/jpeg`,`image/png`,`image/webp`],c=[`pdf`,`jpg`,`jpeg`,`png`,`webp`],l=({nombre:e,apellido:t,correo:n,movil:r,mensaje:i,hasInvoice:a})=>{let o=[`Hola, soy ${[e,t].filter(Boolean).join(` `)}.`,``,`Me gustaría recibir una oferta personalizada para ahorrar en mi factura.`,``,`📞 Teléfono: ${r}`];return n&&o.push(`📧 Correo: ${n}`),i&&o.push(``,`💬 Mensaje:`,i),a&&o.push(``,`📎 También he enviado mi factura a través del formulario.`),o.push(``,`He enviado esta solicitud desde la web de Ahorra Sin Líos.`),o.join(`
`)},u=()=>{let e=t.value?.querySelector(`.ahorra-scope-form form.form`);if(!e)return;let n=e.querySelector(`button[type="submit"]`),i=e.querySelector(`[data-contact-status]`),a=e.elements.factura,o=n.textContent.trim(),u=(e,t)=>{i.textContent=e,i.dataset.state=t,i.hidden=!1},d=()=>{let e=a.files?.[0];if(a.setCustomValidity(``),!e)return!0;let t=e.name.split(`.`).pop()?.toLowerCase();return e.size>No||!s.includes(e.type)||!c.includes(t)?(a.setCustomValidity(e.size>No?`La factura no puede superar los 10 MB.`:`La factura debe ser PDF, JPG, PNG o WEBP.`),!1):!0},f=()=>{d(),a.reportValidity()},p=async t=>{if(t.preventDefault(),i.hidden=!0,!d()||!e.reportValidity())return;n.disabled=!0,n.textContent=`Enviando...`,e.setAttribute(`aria-busy`,`true`);let r=window.open(`about:blank`,`_blank`);r&&(r.opener=null);try{let t=await fetch(jo,{method:`POST`,headers:{Accept:`application/json`},body:new FormData(e)}),n=await t.json().catch(()=>({}));if(!t.ok){let e=Object.values(n.errors??{}).flat()[0],r=Error(`Contact request failed`);throw r.userMessage=t.status===422?e:t.status===503?n.message:null,r}let i=new FormData(e),o=l({nombre:i.get(`nombre`).trim(),apellido:i.get(`apellido`).trim(),correo:i.get(`correo`).trim(),movil:i.get(`movil`).trim(),mensaje:i.get(`mensaje`).trim(),hasInvoice:!!a.files?.length});u(`Solicitud enviada correctamente. Abriendo WhatsApp...`,`success`),window.setTimeout(()=>{let e=`${Mo}?text=${encodeURIComponent(o)}`;if(r&&!r.closed){r.location.replace(e);return}window.open(e,`_blank`,`noopener,noreferrer`)||window.location.assign(e)},500)}catch(t){r?.close(),u(t.userMessage||`No hemos podido enviar tu solicitud. Inténtalo de nuevo en unos segundos.`,`error`),n.disabled=!1,n.textContent=o,e.removeAttribute(`aria-busy`)}};a.addEventListener(`change`,f),e.addEventListener(`submit`,p),r=()=>{a.removeEventListener(`change`,f),e.removeEventListener(`submit`,p)}};Xn(()=>{o(),u()}),$n(()=>{n?.disconnect(),r?.()});let d=`${ko}\n${Ao}\n${Po}`,f=`<div class="ahorra-page">
