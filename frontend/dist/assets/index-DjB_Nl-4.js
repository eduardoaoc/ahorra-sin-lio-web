...
`)},u=()=>{let e=t.value?.querySelector(`.ahorra-scope-form form.form`);if(!e)return;let n=e.querySelector(`button[type="submit"]`),i=e.querySelector(`[data-contact-status]`),a=e.elements.factura,o=n.textContent.trim(),u=(e,t)=>{i.textContent=e,i.dataset.state=t,i.hidden=!1},d=()=>{let e=a.files?.[0];if(a.setCustomValidity(``),!e)return!0;let t=e.name.split(`.`).pop()?.toLowerCase();return e.size>No||!s.includes(e.type)||!c.includes(t)?(a.setCustomValidity(e.size>No?`La factura no puede superar los 10 MB.`:`La factura debe ser PDF, JPG, PNG o WEBP.`),!1):!0},f=()=>{d(),a.reportValidity()},p=async t=>{if(t.preventDefault(),i.hidden=!0,!(!d()||!e.reportValidity())){n.disabled=!0,n.textContent=`Enviando...`,e.setAttribute(`aria-busy`,`true`);try{let t=await fetch(jo,{method:`POST`,headers:{Accept:`application/json`},body:new FormData(e)}),n=await t.json().catch(()=>({}));if(!t.ok){let e=Object.values(n.errors??{}).flat()[0],r=Error(`Contact request failed`);throw r.userMessage=t.status===422?e:t.status===503?n.message:null,r}let r=new FormData(e),i=l({nombre:r.get(`nombre`).trim(),apellido:r.get(`apellido`).trim(),correo:r.get(`correo`).trim(),movil:r.get(`movil`).trim(),mensaje:r.get(`mensaje`).trim(),hasInvoice:!!a.files?.length});u(`Solicitud enviada correctamente. Abriendo WhatsApp...`,`success`),window.setTimeout(()=>{window.location.assign(`${Mo}?text=${encodeURIComponent(i)}`)},500)}catch(t){u(t.userMessage||`No hemos podido enviar tu solicitud. Inténtalo de nuevo en unos segundos.`,`error`),n.disabled=!1,n.textContent=o,e.removeAttribute(`aria-busy`)}}};a.addEventListener(`change`,f),e.addEventListener(`submit`,p),r=()=>{a.removeEventListener(`change`,f),e.removeEventListener(`submit`,p)}};Xn(()=>{o(),u()}),$n(()=>{n?.disconnect(),r?.()});let d=`${ko}\n${Ao}\n${Po}`,f=`<div class="ahorra-page">
